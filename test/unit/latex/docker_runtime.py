...
from ai1_gen.latex import docker_runtime as dr


def test_project_root_from_this_file_points_to_project_root():
    root = dr._project_root_from_this_file()

    assert root.name == "ai1_gen"
    assert (root / "src").exists() or root.is_dir()
...
def test_container_exists_detects_exact_name(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args=args,
            returncode=0,
            stdout="ai1_gen_latex_renderer\n",
            stderr="",
        ),
    )

    assert dr.container_exists("ai1_gen_latex_renderer") is True
...
def test_container_running_detects_exact_name(monkeypatch: pytest.MonkeyPatch):
    ...
    assert dr.container_running("ai1_gen_latex_renderer") is True
...
def test_ensure_latex_container_builds_missing_image_and_starts_container(
    monkeypatch: pytest.MonkeyPatch,
):
    events = []
    ...
    monkeypatch.setattr(dr, "image_exists", lambda image_name: False)
    ...
    monkeypatch.setattr(dr, "build_latex_image", lambda **kwargs: events.append(("build", kwargs)))
    monkeypatch.setattr(dr, "start_latex_container", lambda **kwargs: events.append(("start", kwargs)))
    monkeypatch.setattr(dr, "wait_for_latex_health", lambda **kwargs: True)

    dr.ensure_latex_container(
...
def test_ensure_latex_container_removes_existing_unhealthy_container(
    monkeypatch: pytest.MonkeyPatch,
):
    events = []
    ...
    monkeypatch.setattr(dr, "_health_ok", lambda **kwargs: False)
    monkeypatch.setattr(dr, "docker_available", lambda: True)
    monkeypatch.setattr(dr, "image_exists", lambda image_name: True)
    monkeypatch.setattr(dr, "container_exists", lambda container_name: True)
    monkeypatch.setattr(dr, "container_running", lambda container_name: False)
    monkeypatch.setattr(dr, "remove_latex_container", lambda **kwargs: events.append(("remove", kwargs)))
    monkeypatch.setattr(dr, "start_latex_container", lambda **kwargs: events.append(("start", kwargs)))
    monkeypatch.setattr(dr, "wait_for_latex_health", lambda **kwargs: True)
...
def test_ensure_latex_container_raises_with_logs_when_health_timeout(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(dr, "_health_ok", lambda **kwargs: False)
    monkeypatch.setattr(dr, "docker_available", lambda: True)
    ...
    monkeypatch.setattr(dr, "container_exists", lambda container_name: False)
    monkeypatch.setattr(dr, "container_running", lambda container_name: False)
    monkeypatch.setattr(dr, "start_latex_container", lambda **kwargs: None)
    ...
    with pytest.raises(dr.LatexDockerRuntimeError, match="docker/latex-health-timeout"):
        dr.ensure_latex_container(container_name="c", image_name="img")