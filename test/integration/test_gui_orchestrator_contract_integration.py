...
@pytest.mark.integration
def test_desktop_gui_build_run_input_contract(qtbot, monkeypatch):
    ...
    win.orchestrator = DummyOrchestrator()

    win.config_path_edit.setText("configs/default.yaml")
    win.out_root_edit.setText("D:/desktop_out")
    win.pages_spin.setValue(4)
    win.workers_spin.setValue(1)
    win.seed_spin.setValue(456)
    win.smoke_checkbox.setChecked(False)

    run_input = win.build_run_input()

    assert run_input.config_path.endswith("default.yaml")
    assert run_input.out_root.replace("\\", "/").endswith("D:/desktop_out") or "desktop_out" in run_input.out_root
    assert run_input.pages == 4
    assert run_input.workers == 1
    assert run_input.seed == 456

    overrides = win.collect_overrides()
    assert isinstance(overrides, dict)
    assert "content.block_mix" in overrides