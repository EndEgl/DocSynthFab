...
def test_worker_generate_validate_save_success_writes_outputs(tmp_path, monkeypatch):
    cfg = CfgStub()
    ...
    assert res["page_id"] == "000001"
    assert "hello" in res["jsonl_line"]
    assert (out_root / "images" / "000001.png").exists()
    assert (out_root / "masks" / "000001_mask_text.png").exists()
    assert (out_root / "masks" / "000001_mask_math.png").exists()
    assert (out_root / "ann" / "000001.json").exists()
    assert (out_root / "gt" / "000001.json").exists()

    gt_obj = json.loads((out_root / "gt" / "000001.json").read_text(encoding="utf-8"))
    assert gt_obj["page_id"] == "000001"