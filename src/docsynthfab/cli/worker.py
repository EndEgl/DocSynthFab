...
from docsynthfab.io.exporter import save_json, save_png_u8
from docsynthfab.layout.layout_sampler import sample_page_spec
from docsynthfab.qc.validators import validate_page
from docsynthfab.render.page_renderer import render_page_layers
...
def worker_generate_validate_save(
    ...
    max_tries = int(options.get("max_tries", 8))
    max_tries = max(1, min(max_tries, 20))

    disable_augment_on_try = int(options.get("disable_augment_on_try", 2))
    disable_augment_on_try = max(1, min(disable_augment_on_try, max_tries))
