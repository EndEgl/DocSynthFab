...
#
# Therefore:
#   _PKG_DIR      = .../src/docsynthfab
#   _SRC_ROOT     = .../src
#   _PROJECT_ROOT = .../docsynthfab
...
from docsynthfab.gui.web.simple_controls import (
    CONTENT_MIX_PRESETS,
...
from docsynthfab.orchestrator.result_store import tail_text
...
from docsynthfab.gui.shared.paths import (
    DEFAULT_CONFIG,
    GUI_EVENT_LOG_PATH,
...
from docsynthfab.gui.shared.font_status import (
    fonts_root_path_text,
    format_font_status_text,
    inspect_font_setup,
    missing_font_items,
)

from docsynthfab.gui.shared.latex_setup import (
    DEFAULT_LATEX_HTTP_BASE_URL,
    docker_command_text,
    format_latex_status_text,
    inspect_latex_setup,
    latex_mix_label,
    latex_renderer_url_text,
    latex_run_overrides,
    normalize_latex_mix,
    short_latex_help_text,
)

from docsynthfab.gui.web.config_preview import (
    collect_overrides,
    load_baseline_and_user_config,
    refresh_effective_yaml_preview,
)
from docsynthfab.gui.web.content_actions import (
    ensure_custom_content_csvs,
    reset_content_csvs,
    save_uploaded_content_csv,
)
from docsynthfab.gui.web.live_events import append_gui_event, refresh_live_event_log, safe_notify
...
def _refresh_preview(state: WebGuiState) -> None:
    ...
    layout_randomness_percent = (
        state.layout_randomness_percent_input.value
        if state.layout_randomness_percent_input is not None
        else 25
    )
    ...
    if state.simple_summary_label is not None:
        state.simple_summary_label.text = (
            ...
            f"layout randomness {clamp_percent(layout_randomness_percent, 25):.0f}%."
        )
...
def _refresh_fonts_status(state: WebGuiState) -> None:
    try:
        status = inspect_font_setup()
        missing = missing_font_items(status)

        if state.fonts_root_label is not None:
            state.fonts_root_label.text = f"Fonts root: {status.fonts_root}"

        if state.fonts_status_label is not None:
            state.fonts_status_label.value = format_font_status_text(status)

        if state.fonts_manifest_status_label is not None:
            state.fonts_manifest_status_label.text = (
                "FONT_MANIFEST.json: OK"
                if status.manifest_exists
                else "FONT_MANIFEST.json: MISSING"
            )

        if state.fonts_license_status_label is not None:
            if status.licenses_dir_exists:
                state.fonts_license_status_label.text = (
                    f"LICENSES: OK ({status.license_file_count} files)"
                )
            else:
                state.fonts_license_status_label.text = "LICENSES: MISSING"

        if state.fonts_missing_label is not None:
            if missing:
                state.fonts_missing_label.text = "Missing / attention: " + ", ".join(missing)
            else:
                state.fonts_missing_label.text = "Font setup looks OK."

    except Exception as e:
        if state.fonts_status_label is not None:
            state.fonts_status_label.value = f"Font status check failed: {e!r}"

        if state.fonts_missing_label is not None:
            state.fonts_missing_label.text = f"Font status check failed: {e}"

def _open_fonts_folder() -> None:
    try:
        open_path(fonts_root_path_text())
    except Exception as e:
        ui.notify(f"Open fonts folder failed: {e}", color="negative")


def _refresh_latex_mix_label(state: WebGuiState) -> None:
    try:
        mix = normalize_latex_mix(
            state.latex_text_mix_input.value if state.latex_text_mix_input is not None else 40,
            state.latex_table_mix_input.value if state.latex_table_mix_input is not None else 20,
            state.latex_latex_mix_input.value if state.latex_latex_mix_input is not None else 40,
        )

        if state.latex_generation_total_label is not None:
            state.latex_generation_total_label.text = latex_mix_label(mix)

    except Exception as e:
        if state.latex_generation_total_label is not None:
            state.latex_generation_total_label.text = f"LaTeX mix preview failed: {e}"


def _refresh_latex_status(state: WebGuiState) -> None:
    try:
        base_url = (
            str(state.latex_http_base_url_input.value or DEFAULT_LATEX_HTTP_BASE_URL)
            if state.latex_http_base_url_input is not None
            else DEFAULT_LATEX_HTTP_BASE_URL
        )

        status = inspect_latex_setup(base_url)

        if state.latex_status_label is not None:
            state.latex_status_label.value = format_latex_status_text(status)

        if state.latex_setup_status_label is not None:
            state.latex_setup_status_label.text = (
                "LaTeX renderer: OK"
                if status.ok
                else "LaTeX renderer: needs attention"
            )

        if state.latex_setup_commands_label is not None:
            state.latex_setup_commands_label.value = docker_command_text(port=status.port)

    except Exception as e:
        if state.latex_status_label is not None:
            state.latex_status_label.value = f"LaTeX status check failed: {e!r}"

        if state.latex_setup_status_label is not None:
            state.latex_setup_status_label.text = f"LaTeX status check failed: {e}"


def _collect_latex_run_overrides(state: WebGuiState) -> Dict[str, Any]:
    base_url = (
        str(state.latex_http_base_url_input.value or DEFAULT_LATEX_HTTP_BASE_URL)
        if state.latex_http_base_url_input is not None
        else DEFAULT_LATEX_HTTP_BASE_URL
    ...
    missing_behavior = (
        str(state.latex_missing_behavior_select.value or "fallback")
        if state.latex_missing_behavior_select is not None
        else "fallback"
    )

    return latex_run_overrides(
        text_value=state.latex_text_mix_input.value if state.latex_text_mix_input is not None else 40,
        table_value=state.latex_table_mix_input.value if state.latex_table_mix_input is not None else 20,
        latex_value=state.latex_latex_mix_input.value if state.latex_latex_mix_input is not None else 40,
...
def _start_latex_run(state: WebGuiState) -> None:
    append_gui_event(state, "LaTeX start button clicked.", level="INFO")

    with state.run_lock:
        if current_run_is_active(state):
            append_gui_event(
                state,
                "LaTeX start blocked: active run already exists.",
                level="WARN",
            )
        ...
        if not Path(config_path).exists():
            append_gui_event(
                state,
                f"LaTeX start blocked: config file does not exist: {config_path}",
                level="ERROR",
            )
            safe_notify(
                state,
        ...
        advanced = collect_overrides(state, SCHEMA_MAP)
        simple = _collect_simple_overrides(state)
        latex_overrides = _collect_latex_run_overrides(state)

        overrides: Dict[str, Any] = {}
        overrides.update(advanced)
        overrides.update(simple)
        overrides.update(latex_overrides)
        ...
        try:
            ...
            _set_text(state.progress_label, "LaTeX run started")

            _set_running_ui(state, True)

            append_gui_event(state, f"LaTeX run started: {run_id}", level="INFO")
            safe_notify(state, f"LaTeX run started: {run_id}", color="positive", level="INFO")

            _refresh_status(state)

        except Exception as e:
            _set_running_ui(state, False)
            append_gui_event(state, f"LaTeX run start failed: {e!r}", level="ERROR")
            safe_notify(state, f"LaTeX run start failed: {e}", color="negative", level="ERROR")
...
def _stop_run(state: WebGuiState) -> None:
    ...
    try:
        ...
        from docsynthfab.gui.web.run_state import is_unknown_run_error, reset_lost_run_state
    ...
    with ui.card().classes("ai-card w-full"):
        with ui.row().classes("w-full items-center justify-between gap-3"):
            with ui.row().classes("items-center gap-4"):
                ui.image("/assets/images/docsynthfab-logo.png").classes(
                    "w-24 h-24 object-contain rounded-xl"
                )

                with ui.column().classes("gap-0"):
                    ui.label("DocSynthFab").classes("text-2xl font-bold text-gray-900")
...
def _build_dataset_controls_card(state: WebGuiState) -> None:
    with ui.card().classes("ai-card w-full"):
        _section_header(
            "1. Presets",
            "Choose the dataset goal, visual character, and text/table balance.",
        )
...
def _build_content_mix_card(state: WebGuiState) -> None:
    with ui.card().classes("ai-card w-full"):
        _section_header(
            "2. Custom text/table mix",
            "Used only when Content balance is set to Custom. Values are normalized to 100%.",
        )
...
def _build_layout_behavior_card(state: WebGuiState) -> None:
    with ui.card().classes("ai-card w-full"):
        _section_header(
            "3. Main generation controls",
            "Control text source, text length, table size, density and layout behavior.",
        )

        with ui.expansion("Content and diversity", icon="dashboard_customize").classes("w-full"):
            with ui.grid(columns=2).classes("w-full gap-3"):
                ...
                state.text_length_select = ui.select(
                    ...
                    label="Text length preset",
                ).classes("w-full")

                state.content_source_mode_select = ui.select(
                    options={
                        "word_bank": "Word bank synthetic",
                        "sentence_bank": "Sentence bank natural",
                        "mixed_bank": "Mixed word/sentence bank",
                        "random_chars": "Random characters / stress",
                    },
                    value="word_bank",
            ...
            ui.label(
                "Text length values override the selected preset. "
                "Word bank mode is recommended for multilingual OCR/layout datasets."
            ).classes("text-xs text-gray-500")

            with ui.grid(columns=2).classes("w-full gap-3"):
                state.text_min_words_input = ui.number(
                    "Text min words",
                    value=25,
                    min=1,
                    max=5000,
                ...
                state.text_max_words_input = ui.number(
                    "Text max words",
                    value=90,
                    min=1,
                    max=10000,
                ...
                state.sentence_min_input = ui.number(
                    "Sentence min",
                    value=2,
                    min=1,
                    max=200,
                ...
                state.sentence_max_input = ui.number(
                    "Sentence max",
                    value=6,
                    min=1,
                    max=500,
        ...
        with ui.expansion("Readable / hard examples", icon="tune").classes("w-full"):
            with ui.grid(columns=2).classes("w-full gap-3"):
                state.font_size_profile_select = ui.select(
                    options=["Readable", "Balanced", "Small / Hard", "Custom"],
                    value="Balanced",
                    label="Font size",
                ).classes("w-full")

                state.font_min_px_input = ui.number(
                    "Font min px",
                    value=10,
                    min=4,
                    max=96,
                ...
                state.font_max_px_input = ui.number(
                    "Font max px",
                    value=18,
                    min=4,
                    max=128,
                ...
                state.layout_randomness_percent_input = ui.number(
                    "Layout randomness (%)",
                    value=25,
                    min=0,
                    max=100,
                ...
                state.negative_space_profile_select = ui.select(
                    options=[
                        "Airy",
                        "Balanced",
                        "Controlled",
                    ...
                    value="Controlled",
                    label="Negative space control",
                ).classes("w-full")

            ui.label(
                "Table size values are sent as backend config overrides. "
                "They are applied by the table/layout generator when table blocks are selected."
            ).classes("text-xs text-gray-500")

            with ui.grid(columns=2).classes("w-full gap-3"):
                state.table_min_rows_input = ui.number(
                    "Table min rows",
                    value=6,
                    min=1,
                    max=200,
                ...
                state.table_max_rows_input = ui.number(
                    "Table max rows",
                    value=18,
                    min=1,
                    max=500,
                ...
                state.table_min_cols_input = ui.number(
                    "Table min cols",
                    value=3,
                    min=1,
                    max=80,
                ...
                state.table_max_cols_input = ui.number(
                    "Table max cols",
                    value=10,
                    min=1,
                    max=120,
        ...
        for widget in (
            ...
            state.text_min_words_input,
            state.text_max_words_input,
            state.sentence_min_input,
            state.sentence_max_input,
            state.font_size_profile_select,
            state.font_min_px_input,
            state.font_max_px_input,
            state.density_percent_input,
            state.layout_randomness_percent_input,
            state.negative_space_profile_select,
            state.table_min_rows_input,
            state.table_max_rows_input,
            state.table_min_cols_input,
            state.table_max_cols_input,
        ):
            widget.on("update:model-value", lambda *_: _refresh_preview(state))
...
def _build_fonts_status_card(state: WebGuiState) -> None:
    with ui.card().classes("ai-card w-full"):
        _section_header(
            "Fonts Status",
            "Check bundled font folders, FONT_MANIFEST.json, and license files. Font selection is automatic by script/alphabet.",
        )

        state.fonts_root_label = ui.label("-").classes("text-sm text-gray-600")
        state.fonts_manifest_status_label = ui.label("-").classes("text-sm text-gray-600")
        state.fonts_license_status_label = ui.label("-").classes("text-sm text-gray-600")
        state.fonts_missing_label = ui.label("-").classes("text-sm text-gray-600")

        state.fonts_status_label = ui.textarea(
            value="Not checked yet.",
        ).classes("w-full").props("readonly autogrow")

        with ui.row().classes("gap-2"):
            state.fonts_check_button = ui.button(
                "Check font setup",
                icon="fact_check",
                on_click=lambda: _refresh_fonts_status(state),
            )

            state.fonts_open_button = ui.button(
                "Open fonts folder",
                icon="folder_open",
                on_click=_open_fonts_folder,
            )

        _refresh_fonts_status(state)
...
def _build_templates_content_tab(state: WebGuiState) -> None:
    with ui.row().classes("w-full gap-4 items-start"):
        with ui.column().classes("w-full gap-4"):
            ...
            _build_fonts_status_card(state)

    # Public v1:
    # Fixed-form template CSV UI is intentionally hidden.
    # It may return later as an optional business/template-pack feature.

def _build_latex_renderer_tab(state: WebGuiState) -> None:
    with ui.row().classes("w-full gap-4 items-start"):
        with ui.column().classes("w-full lg:w-6/12 gap-4"):
            with ui.card().classes("ai-card w-full"):
                _section_header(
                    "LaTeX Renderer",
                    "Dedicated LaTeX/math-heavy generation. The main generator remains Text/Table-only.",
                )

                ui.label(short_latex_help_text()).classes("text-sm text-gray-600")

                state.latex_http_base_url_input = ui.input(
                    "Renderer URL",
                    value=latex_renderer_url_text(DEFAULT_LATEX_HTTP_BASE_URL),
                ).classes("w-full")

                state.latex_missing_behavior_select = ui.select(
                    options={
                        "fallback": "Fallback when missing",
                        "skip": "Skip LaTeX blocks when missing",
                        "error": "Fail if renderer is missing",
                    },
                    value="fallback",
                    label="Missing behavior",
                ).classes("w-full")

                state.latex_setup_status_label = ui.label("-").classes("text-sm text-gray-600")

                with ui.row().classes("gap-2"):
                    ui.button(
                        "Check renderer",
                        icon="fact_check",
                        on_click=lambda: _refresh_latex_status(state),
                    )
                    ...
                    ui.button(
                        ...
                        icon="play_arrow",
                        color="positive",
                        on_click=lambda: _start_latex_run(state),
                    )

            with ui.card().classes("ai-card w-full"):
                _section_header(
                    "LaTeX content mix",
                    "Used only by Start LaTeX run. Values are normalized to 100%.",
                )

                with ui.grid(columns=3).classes("w-full gap-3"):
                    state.latex_text_mix_input = ui.number(
                        "Text (%)",
                        value=40,
                        min=0,
                        max=100,
                    ...
                    state.latex_table_mix_input = ui.number(
                        "Table (%)",
                        value=20,
                        min=0,
                        max=100,
                    ...
                    state.latex_latex_mix_input = ui.number(
                        "LaTeX (%)",
                        value=40,
                        min=0,
                        max=100,
                ...
                state.latex_generation_total_label = ui.label("-").classes("text-sm text-gray-700")

                for widget in (
                    state.latex_text_mix_input,
                    state.latex_table_mix_input,
                    state.latex_latex_mix_input,
                ):
                    widget.on("update:model-value", lambda *_: _refresh_latex_mix_label(state))

                _refresh_latex_mix_label(state)

        with ui.column().classes("w-full lg:w-6/12 gap-4"):
            with ui.card().classes("ai-card w-full"):
                _section_header(
                    "Renderer status",
                    "Checks Docker, port availability, and /health endpoint.",
                )

                state.latex_status_label = ui.textarea(
                    value="Not checked yet.",
                ).classes("w-full").props("readonly autogrow")
            ...
            with ui.card().classes("ai-card w-full"):
                _section_header(
                    ...
                    "Use these commands if the renderer is not running yet.",
                )

                state.latex_setup_commands_label = ui.textarea(
                    value=docker_command_text(),
                ).classes("w-full").props("readonly autogrow")

    _refresh_latex_status(state)
...
def build_ui() -> None:
    ...
    app.add_static_files("/assets", str(_PROJECT_ROOT / "assets"))

    _install_page_style()
    ui.page_title("DocSynthFab Web GUI")

    with ui.header().classes("items-center justify-between"):
        with ui.row().classes("items-center gap-3"):
            ui.icon("description").classes("text-2xl")
            ui.label("DocSynthFab").classes("text-xl font-bold")
    ...
    with ui.column().classes("ai-page gap-4"):
        ...
        with ui.tabs().classes("w-full") as main_tabs:
            ...
            tab_templates = ui.tab("Content", icon="table_chart")
            tab_latex = ui.tab("LaTeX Renderer", icon="functions")
        ...
        with ui.tab_panels(main_tabs, value=tab_create).classes("w-full"):
            ...
            with ui.tab_panel(tab_latex):
                _build_latex_renderer_tab(state)
