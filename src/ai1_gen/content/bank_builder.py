# src/ai1_gen/content/bank_builder.py
...
def _load_words_csv(path: Path) -> List[Dict[str, Any]]:
    if not path.exists():
        return []
...
def _collect_label_registry(
    ...
    out: List[Dict[str, Any]] = []
    for kind, value in sorted(seen, key=lambda x: (x[0], x[1])):
...
def build_content_bank_json(
    *,
    ...
    words = _load_words_csv(words_path)
